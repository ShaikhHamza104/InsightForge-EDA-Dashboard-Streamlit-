...
import os
# Suppress Streamlit warnings with correct environment variables
...
# 3) Header with fixed emoji rendering
st.markdown(
    '<h1 class="main-header">📊 InsightForge — EDA Dashboard</h1>', 
    unsafe_allow_html=True
)
st.markdown(
    '<p class="subtitle">Analyze datasets quickly with interactive Overview, Univariate, and Bivariate visualizations</p>', 
    unsafe_allow_html=True
)
...
with st.sidebar:
    st.markdown('<h2 class="sidebar-title">📊 InsightForge</h2>', unsafe_allow_html=True)
    ...
    st.markdown("---")
    st.markdown('<p class="footer-text">Built with ❤️ using Streamlit</p>', unsafe_allow_html=True)
